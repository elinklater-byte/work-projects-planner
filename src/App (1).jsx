...
const TASK_STATES = ['To Do', 'Scheduled', 'In Progress', 'Completed']; // per-task/subtask state
...
function stateColor(s) {
  if (s === 'Completed') return { fg: COLORS.pine, bg: COLORS.pineLight };
  if (s === 'In Progress') return { fg: COLORS.amber, bg: COLORS.amberLight };
  if (s === 'Scheduled') return { fg: COLORS.sky, bg: COLORS.skyLight };
  return { fg: COLORS.inkSoft, bg: COLORS.paperDark }; // To Do
}
...
// Older saved data only has a boolean `done`. Newer data has a `state` from
// TASK_STATES. This derives one from the other so nothing has to be migrated
...
function isItemDone(item) {
  return getItemState(item) === 'Completed';
}

function toggledState(item) {
  return isItemDone(item) ? 'To Do' : 'Completed';
}
...
export default function WorkBoard() {
  ...
  function addTask(projectId, text) {
    ...
    persistProjects(projects.map(p => (
      p.id === projectId
        ? { ...p, tasks: [...p.tasks, { id: uid(), text: text.trim(), state: 'To Do', dueDate: '', notes: '', subtasks: [] }] }
        : p
    )));
  }
  ...
  function addSubtask(projectId, taskId, text) {
    ...
    persistProjects(projects.map(p => {
      ...
      return {
        ...
        tasks: p.tasks.map(t => (
          t.id === taskId
            ? { ...t, subtasks: [...(t.subtasks || []), { id: uid(), text: text.trim(), state: 'To Do', dueDate: '', subtasks: [] }] }
            : t
        )),
      };
    }));
        ...
        tasks: p.tasks.map(t => {
          ...
          return {
            ...
            subtasks: (t.subtasks || []).map(s => (
              s.id === subtaskId
                ? { ...s, subtasks: [...(s.subtasks || []), { id: uid(), text: text.trim(), state: 'To Do', dueDate: '' }] }
                : s
            )),
          };
        }),
...
function TreeRow({ item, depth, onToggle, onEditText }) {
  const state = getItemState(item);
  const sc = stateColor(state);
  const due = formatDue(item.dueDate);
  const overdue = due && due.overdue && state !== 'Completed';
  return (
    <div className="flex items-center gap-2 py-1" style={{ paddingLeft: depth * 20 }}>
      ...
      <EditableText
        ...
        style={{ color: state === 'Completed' ? COLORS.inkSoft : COLORS.ink, textDecoration: state === 'Completed' ? 'line-through' : 'none' }}
      />
      <span className="wb-mono text-[10px] px-1.5 py-0.5 rounded shrink-0" style={{ backgroundColor: sc.bg, color: sc.fg }}>{state}</span>
      {due && (
        <span className="wb-mono text-[10px] shrink-0" style={{ color: overdue ? COLORS.rose : COLORS.inkSoft }}>{due.label}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg border" style={{ borderColor: COLORS.border }}>
      <div className="flex items-center gap-2 px-2 py-2 group">
        ...
        />
        {childTotal > 0 && (
          <span className="wb-mono text-xs" style={{ color: COLORS.inkSoft }}>{childDone}/{childTotal}</span>
        )}
        {due && (
          <span className="wb-mono text-xs flex items-center gap-1" style={{ color: overdue ? COLORS.rose : COLORS.inkSoft }}>
            <Calendar size={11} /> {due.label}
          </span>
        )}
        <button onClick={() => setExpanded(e => !e)} style={{ color: COLORS.inkSoft }}>
          {expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
        </button>
    ...
    <div className="rounded-md border" style={{ borderColor: COLORS.border }}>
      ...
      {expanded && (
        <div className="px-2 pb-2 pt-1 border-t flex flex-col gap-1.5" style={{ borderColor: COLORS.border }}>
          {subsubtasks.map(ss => {
            const ssState = getItemState(ss);
            const ssDue = formatDue(ss.dueDate);
            const ssOverdue = ssDue && ssDue.overdue && ssState !== 'Completed';
            return (
              <div key={ss.id} className="flex items-center gap-2 pl-4 flex-wrap">
                <button onClick={() => onUpdateSubSubtask(ss.id, { state: toggledState(ss) })}>
                  {ssState === 'Completed'
                ...
                <EditableText
                  value={ss.text}
                  onCommit={text => onUpdateSubSubtask(ss.id, { text })}
                  className="text-[11px] flex-1 min-w-[70px]"
                  style={{ color: ssState === 'Completed' ? COLORS.inkSoft : COLORS.ink, textDecoration: ssState === 'Completed' ? 'line-through' : 'none' }}
                />
                <input
                  type="date"
                  value={ss.dueDate || ''}
                  onChange={e => onUpdateSubSubtask(ss.id, { dueDate: e.target.value })}
                  className="wb-mono text-[10px] rounded px-1 py-0.5 border outline-none"
                  style={{ borderColor: COLORS.border, color: ssOverdue ? COLORS.rose : COLORS.inkSoft, width: 100 }}
                />
                <StateSelect value={ssState} onChange={s => onUpdateSubSubtask(ss.id, { state: s })} />
                <button onClick={() => onDeleteSubSubtask(ss.id)}><X size={11} style={{ color: COLORS.inkSoft }} /></button>
              </div>
            );
          })}
