...
import {
  ...
  ChevronDown, ChevronUp, List as ListIcon, Link2,
...
export default function WorkBoard() {
  ...
  // Item 2: drag-to-reorder in list view. `newOrderIds` is the desired order
  // for whichever projects are currently visible (i.e. match the active
  // search) — anything hidden by a filter keeps its exact original slot so
  // reordering while searching never shuffles unrelated projects around.
  function reorderProjects(newOrderIds) {
    const byId = new Map(projects.map(p => [p.id, p]));
    const visible = new Set(newOrderIds);
    let cursor = 0;
    const result = projects.map(p => (visible.has(p.id) ? byId.get(newOrderIds[cursor++]) : p));
  ...
  function addTask(projectId, text) {
    ...
    persistProjects(projects.map(p => (
      p.id === projectId
        ? { ...p, tasks: [...p.tasks, { id: uid(), text: text.trim(), state: 'Not Scheduled', priority: 'No Priority', dueDate: '', notes: '', driveLink: '', subtasks: [] }] }
        : p
    )));
  }
  ...
  function addSubtask(projectId, taskId, text) {
    ...
    persistProjects(projects.map(p => {
      ...
      return {
        ...
        tasks: p.tasks.map(t => (
          t.id === taskId
            ? { ...t, subtasks: [...(t.subtasks || []), { id: uid(), text: text.trim(), state: 'Not Scheduled', priority: 'No Priority', dueDate: '', notes: '', driveLink: '', subtasks: [] }] }
            : t
        )),
      };
    }));
        ...
        tasks: p.tasks.map(t => {
          ...
          return {
            ...
            subtasks: (t.subtasks || []).map(s => (
              s.id === subtaskId
                ? { ...s, subtasks: [...(s.subtasks || []), { id: uid(), text: text.trim(), state: 'Not Scheduled', priority: 'No Priority', dueDate: '', notes: '', driveLink: '' }] }
                : s
            )),
          };
        }),
  ...
  // Depth is fixed at 3 levels (Task -> Subtask -> Sub-subtask). Moving an
  // item one level deeper is only safe when it has no children that would
  // themselves need to land a level past the bottom — those moves are
  // guarded here and also kept off the menu in the UI.

  // Task -> becomes a Subtask nested under a different Task.
  function moveTaskUnderTask(projectId, taskId, targetTaskId) {
    if (taskId === targetTaskId) return;
    persistProjects(projects.map(p => {
      if (p.id !== projectId) return p;
      const task = p.tasks.find(t => t.id === taskId);
      if (!task) return p;
      const tooDeep = (task.subtasks || []).some(s => (s.subtasks || []).length > 0);
      if (tooDeep) return p;
      const remaining = p.tasks.filter(t => t.id !== taskId);
      return {
        ...p,
        tasks: remaining.map(t => (t.id === targetTaskId ? { ...t, subtasks: [...(t.subtasks || []), task] } : t)),
      };
    }));
  }

  // Subtask -> promoted up to a top-level Task.
  function promoteSubtaskToTask(projectId, taskId, subtaskId) {
    persistProjects(projects.map(p => {
      if (p.id !== projectId) return p;
      const task = p.tasks.find(t => t.id === taskId);
      const sub = task && (task.subtasks || []).find(s => s.id === subtaskId);
      if (!sub) return p;
      const tasksWithoutSub = p.tasks.map(t => (
        t.id === taskId ? { ...t, subtasks: (t.subtasks || []).filter(s => s.id !== subtaskId) } : t
      ));
      return { ...p, tasks: [...tasksWithoutSub, sub] };
    }));
  }

  // Subtask -> moved to become a subtask of a different Task (same depth).
  function moveSubtaskUnderTask(projectId, taskId, subtaskId, targetTaskId) {
    if (taskId === targetTaskId) return;
    persistProjects(projects.map(p => {
      if (p.id !== projectId) return p;
      const task = p.tasks.find(t => t.id === taskId);
      ...
      return {
        ...p,
        tasks: tasksWithoutSub.map(t => (t.id === targetTaskId ? { ...t, subtasks: [...(t.subtasks || []), sub] } : t)),
      };
    }));
  }

  // Subtask -> nested one level deeper, as a Sub-subtask under a different
  // subtask. Only allowed when the moved subtask has no sub-subtasks of its
  // own (those would have nowhere to go).
  function moveSubtaskUnderSubtask(projectId, taskId, subtaskId, targetTaskId, targetSubtaskId) {
    persistProjects(projects.map(p => {
      if (p.id !== projectId) return p;
      const task = p.tasks.find(t => t.id === taskId);
      const sub = task && (task.subtasks || []).find(s => s.id === subtaskId);
      if (!sub || (sub.subtasks || []).length > 0) return p;
      const { subtasks, ...subAsLeaf } = sub;
      const tasksWithoutSub = p.tasks.map(t => (
        t.id === taskId ? { ...t, subtasks: (t.subtasks || []).filter(s => s.id !== subtaskId) } : t
      ));
      return {
        ...p,
        tasks: tasksWithoutSub.map(t => (
          t.id === targetTaskId
            ? { ...t, subtasks: (t.subtasks || []).map(s => (s.id === targetSubtaskId ? { ...s, subtasks: [...(s.subtasks || []), subAsLeaf] } : s)) }
            : t
        )),
      };
    }));
  ...
  // Sub-subtask -> promoted up to a Subtask under its current parent Task.
  function promoteSubSubtaskToSubtask(projectId, taskId, subtaskId, subSubtaskId) {
    persistProjects(projects.map(p => {
      if (p.id !== projectId) return p;
      const task = p.tasks.find(t => t.id === taskId);
      const sub = task && (task.subtasks || []).find(s => s.id === subtaskId);
      const ss = sub && (sub.subtasks || []).find(x => x.id === subSubtaskId);
      if (!ss) return p;
      const newSub = { ...ss, subtasks: [] };
      return {
        ...p,
        tasks: p.tasks.map(t => (
          t.id !== taskId ? t : {
            ...
            subtasks: [
              ...(t.subtasks || []).map(s => (
                s.id === subtaskId ? { ...s, subtasks: (s.subtasks || []).filter(x => x.id !== subSubtaskId) } : s
              )),
              newSub,
            ],
          }
  ...
  // Sub-subtask -> moved to become a Sub-subtask under a different subtask.
  function moveSubSubtaskUnderSubtask(projectId, taskId, subtaskId, subSubtaskId, targetTaskId, targetSubtaskId) {
    persistProjects(projects.map(p => {
      if (p.id !== projectId) return p;
      const task = p.tasks.find(t => t.id === taskId);
      const sub = task && (task.subtasks || []).find(s => s.id === subtaskId);
      ...
      return {
        ...
        tasks: removed.map(t => (
          t.id !== targetTaskId ? t : {
            ...t,
            subtasks: (t.subtasks || []).map(s => (
              s.id === targetSubtaskId ? { ...s, subtasks: [...(s.subtasks || []), ss] } : s
            )),
          }
        )),
      };
    ...
    <div className="min-h-screen" style={{ backgroundColor: COLORS.paper, color: COLORS.ink }}>
      ...
      {activeProject && (
        <ProjectDetail
          ...
          onMoveTaskUnderTask={(taskId, targetTaskId) => moveTaskUnderTask(activeProject.id, taskId, targetTaskId)}
          onPromoteSubtaskToTask={(taskId, subId) => promoteSubtaskToTask(activeProject.id, taskId, subId)}
          onMoveSubtaskUnderTask={(taskId, subId, targetTaskId) => moveSubtaskUnderTask(activeProject.id, taskId, subId, targetTaskId)}
          onMoveSubtaskUnderSubtask={(taskId, subId, targetTaskId, targetSubId) => moveSubtaskUnderSubtask(activeProject.id, taskId, subId, targetTaskId, targetSubId)}
          onPromoteSubSubtaskToSubtask={(taskId, subId, ssId) => promoteSubSubtaskToSubtask(activeProject.id, taskId, subId, ssId)}
          onMoveSubSubtaskUnderSubtask={(taskId, subId, ssId, targetTaskId, targetSubId) => moveSubSubtaskUnderSubtask(activeProject.id, taskId, subId, ssId, targetTaskId, targetSubId)}
        />
      )}
    </div>
  );
...
function ProjectListView({
  projects, onOpen, onUpdateTask, onUpdateSubtask, onUpdateSubSubtask,
  onAddTask, onAddSubtask, onAddSubSubtask, onReorder,
}) {
  ...
  function submitQuickAdd(id) {
    if (!quickAddText.trim()) return;
    onAddTask(id, quickAddText);
    setQuickAddText('');
    setQuickAddId(null);
    setExpandedIds(e => ({ ...e, [id]: true }));
  }

  // Item 2: drag a project by its grip handle to reorder the list. Reorders
  // only among the currently visible (filtered/searched) projects — anything
  // hidden by the current search stays exactly where it was.
  function handleDrop(targetId) {
    setDragOverId(null);
    if (!dragProjectId || dragProjectId === targetId) { setDragProjectId(null); return; }
    const ids = projects.map(p => p.id);
    const fromIdx = ids.indexOf(dragProjectId);
    if (fromIdx === -1 || !ids.includes(targetId)) { setDragProjectId(null); return; }
    const reordered = [...ids];
    reordered.splice(fromIdx, 1);
    const insertAt = reordered.indexOf(targetId);
    reordered.splice(insertAt, 0, dragProjectId);
    onReorder(reordered);
  ...
  return (
    <div className="flex flex-col gap-2">
      {projects.map(p => {
        ...
        const quickAddOpen = quickAddId === p.id;
        return (
          <div
            key={p.id}
            className="rounded-lg border"
            style={{
              borderColor: dragOverId === p.id ? COLORS.pine : COLORS.border,
              backgroundColor: dragOverId === p.id ? COLORS.pineLight : COLORS.card,
            }}
            onDragOver={e => { if (dragProjectId) { e.preventDefault(); setDragOverId(p.id); } }}
          ...
          >
            <div className="flex items-center gap-3 px-3 py-2.5">
              <span
                ...
                onDragEnd={() => { setDragProjectId(null); setDragOverId(null); }}
              ...
              <button
                ...
                style={{ backgroundColor: quickAddOpen ? COLORS.pine : COLORS.paperDark, color: quickAddOpen ? '#fff' : COLORS.inkSoft }}
              >
                <Plus size={13} />
              </button>
              {total > 0 && (
            ...
            {quickAddOpen && (
              <div className="px-3 pb-2.5 flex gap-2 border-t pt-2" style={{ borderColor: COLORS.border }}>
                <input
                  autoFocus
                  value={quickAddText}
                  onChange={e => setQuickAddText(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') submitQuickAdd(p.id); if (e.key === 'Escape') setQuickAddId(null); }}
                  placeholder="Add a task…"
                  className="flex-1 px-2 py-1.5 rounded-lg border text-sm outline-none"
                  style={{ borderColor: COLORS.border }}
                />
                <button onClick={() => submitQuickAdd(p.id)} className="px-2.5 rounded-lg" style={{ backgroundColor: COLORS.pine, color: '#fff' }}>
                  <Plus size={14} />
                </button>
              </div>
            )}
              ...
              <div className="px-3 pb-3 pt-1 border-t flex flex-col gap-0.5" style={{ borderColor: COLORS.border }}>
                {p.tasks.length === 0 ? (
                  ...
                  <TreeRow
                    ...
                    showDetails
                    onEditNotes={text => onUpdateTask(p.id, t.id, { notes: text })}
                    onEditDriveLink={url => onUpdateTask(p.id, t.id, { driveLink: url })}
                    onAddChild={text => onAddSubtask(p.id, t.id, text)}
                    addChildLabel="Add a subtask…"
                  >
                    {(t.subtasks || []).map(s => (
                      <TreeRow
                        key={s.id}
                        ...
                        showDetails
                        onEditNotes={text => onUpdateSubtask(p.id, t.id, s.id, { notes: text })}
                        onEditDriveLink={url => onUpdateSubtask(p.id, t.id, s.id, { driveLink: url })}
                        onAddChild={text => onAddSubSubtask(p.id, t.id, s.id, text)}
                        addChildLabel="Add a sub-subtask…"
                      >
                        {(s.subtasks || []).map(ss => (
                          <TreeRow
                            key={ss.id}
...
// due date, priority, and status for itself. Task and Subtask rows also get
// a second "details" toggle (Notes, Drive link, add-a-child) so the full
// outline can be built and annotated without leaving list view.
function TreeRow({
  item, depth, onToggle, onEditText, onEditState, onEditPriority, onEditDueDate,
  hasChildren, children,
  showDetails, onEditNotes, onEditDriveLink, onAddChild, addChildLabel,
}) {
  ...
  useEffect(() => { setDriveLink(item.driveLink || ''); }, [item.id]);

  function submitChild() {
    if (!childText.trim()) return;
    onAddChild(childText);
    setChildText('');
  ...
  return (
    <div>
      <div className="flex items-center gap-2 py-1 flex-wrap" style={{ paddingLeft: depth * 20 }}>
        ...
        {showDetails && (
          <button
            ...
            title="Note, link & add"
            className="shrink-0"
            style={{ color: detailsOpen ? COLORS.pine : COLORS.inkSoft }}
          >
      ...
      {showDetails && detailsOpen && (
        <div className="flex flex-col gap-1.5 pb-2 pt-1" style={{ paddingLeft: (depth + 1) * 20 }}>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            onBlur={() => onEditNotes(notes)}
            placeholder="Add a note…"
            rows={2}
            className="w-full px-2 py-1 rounded-md border text-xs outline-none resize-none"
            style={{ borderColor: COLORS.border, maxWidth: 420 }}
          />
          <div className="flex gap-1.5" style={{ maxWidth: 420 }}>
            <input
              type="url"
              value={driveLink}
              onChange={e => setDriveLink(e.target.value)}
              onBlur={() => onEditDriveLink(driveLink)}
            ...
            {item.driveLink && (
              <a
                ...
                rel="noopener noreferrer"
                title="Open linked document"
                className="px-1.5 rounded-md flex items-center"
                style={{ backgroundColor: COLORS.skyLight, color: COLORS.sky }}
              >
                <Link2 size={12} />
              </a>
            )}
          ...
          {onAddChild && (
            <div className="flex gap-1.5" style={{ maxWidth: 420 }}>
              <input
                ...
                placeholder={addChildLabel}
                className="flex-1 px-2 py-1 rounded-md border text-xs outline-none"
                style={{ borderColor: COLORS.border }}
              />
              <button onClick={submitChild} className="px-2 rounded-md" style={{ backgroundColor: COLORS.pine, color: '#fff' }}>
                <Plus size={12} />
              </button>
            </div>
          )}
...
function ProjectDetail({
  ...
  onMoveTaskUnderTask, onPromoteSubtaskToTask, onMoveSubtaskUnderTask,
  onMoveSubtaskUnderSubtask, onPromoteSubSubtaskToSubtask, onMoveSubSubtaskUnderSubtask,
}) {
  ...
  // Flat lookup lists used to populate the "Move…" menus at every level —
  // built once per render rather than threading the whole project tree
  // through every nested row component.
  const allTasksFlat = project.tasks.map(t => ({ id: t.id, text: t.text }));
  const allSubtasksFlat = project.tasks.flatMap(t => (
    (t.subtasks || []).map(s => ({ id: s.id, text: s.text, taskId: t.id, taskText: t.text }))
  ));
          ...
          <div className="flex flex-col gap-2 mt-2">
            {project.tasks.map(t => (
              <TaskItem
                ...
                allSubtasksFlat={allSubtasksFlat}
                onMoveUnderTask={targetTaskId => onMoveTaskUnderTask(t.id, targetTaskId)}
                onPromoteSubtaskToTask={subId => onPromoteSubtaskToTask(t.id, subId)}
                onMoveSubtaskUnderTask={(subId, targetTaskId) => onMoveSubtaskUnderTask(t.id, subId, targetTaskId)}
                onMoveSubtaskUnderSubtask={(subId, targetTaskId, targetSubId) => onMoveSubtaskUnderSubtask(t.id, subId, targetTaskId, targetSubId)}
                onPromoteSubSubtaskToSubtask={(subId, ssId) => onPromoteSubSubtaskToSubtask(t.id, subId, ssId)}
                onMoveSubSubtaskUnderSubtask={(subId, ssId, targetTaskId, targetSubId) => onMoveSubSubtaskUnderSubtask(t.id, subId, ssId, targetTaskId, targetSubId)}
              />
            ))}
          </div>
...
function TaskItem({
  ...
  onAddSubtask, onUpdateSubtask, onDeleteSubtask,
  onAddSubSubtask, onUpdateSubSubtask, onDeleteSubSubtask,
  allTasksFlat, allSubtasksFlat, onMoveUnderTask,
  onPromoteSubtaskToTask, onMoveSubtaskUnderTask, onMoveSubtaskUnderSubtask,
  onPromoteSubSubtaskToSubtask, onMoveSubSubtaskUnderSubtask,
}) {
  ...
  // A task can only be nested as a subtask if none of ITS subtasks already
  // have sub-subtasks of their own — otherwise those would need a 4th level.
  const canNest = subtasks.every(s => (s.subtasks || []).length === 0);
  const moveTargets = (allTasksFlat || []).filter(t => t.id !== task.id);

  useEffect(() => { setNotes(task.notes || ''); }, [task.id]);
  useEffect(() => { setDriveLink(task.driveLink || ''); }, [task.id]);
        ...
        <div className="px-3 pb-3 pt-2 border-t" style={{ borderColor: COLORS.border }}>
          ...
          <div className="flex gap-3 mb-3 flex-wrap">
            <div className="flex-1 min-w-[160px]">
              <label className="wb-mono text-xs uppercase tracking-wide" style={{ color: COLORS.inkSoft }}>Drive link</label>
              <div className="flex gap-1.5 mt-1">
                <input
                  type="url"
                  value={driveLink}
                  onChange={e => setDriveLink(e.target.value)}
                  onBlur={() => onUpdate({ driveLink })}
                  placeholder="Paste a Google Drive link…"
            ...
            {moveTargets.length > 0 && (
              <div className="flex-1 min-w-[160px]">
                <label className="wb-mono text-xs uppercase tracking-wide" style={{ color: COLORS.inkSoft }}>Move in outline</label>
                <select
                  value=""
                  onChange={handleMove}
                  disabled={!canNest}
                  title={canNest ? 'Nest this task as a subtask of another task' : "This task's subtasks already have their own sub-subtasks — simplify it first to nest it deeper"}
                  className="w-full mt-1 px-2 py-1.5 rounded-lg border text-xs outline-none"
                  style={{ borderColor: COLORS.border, color: canNest ? COLORS.ink : COLORS.inkSoft, backgroundColor: canNest ? 'transparent' : COLORS.paperDark }}
                >
                  <option value="" disabled>Make a subtask of…</option>
                  {moveTargets.map(t => <option key={t.id} value={t.id}>{t.text}</option>)}
                </select>
              </div>
            )}
          </div>
            ...
            <div className="flex flex-col gap-1.5 mt-2">
              {subtasks.map(s => (
                <SubtaskRow
                  ...
                  allTasksFlat={allTasksFlat}
                  allSubtasksFlat={allSubtasksFlat}
                  onPromoteToTask={() => onPromoteSubtaskToTask(s.id)}
                  onMoveToTask={targetTaskId => onMoveSubtaskUnderTask(s.id, targetTaskId)}
                  onMoveToSubtask={(targetTaskId, targetSubId) => onMoveSubtaskUnderSubtask(s.id, targetTaskId, targetSubId)}
                  onPromoteSubSubtaskToSubtask={ssId => onPromoteSubSubtaskToSubtask(s.id, ssId)}
                  onMoveSubSubtaskUnderSubtask={(ssId, targetTaskId, targetSubId) => onMoveSubSubtaskUnderSubtask(s.id, ssId, targetTaskId, targetSubId)}
                />
              ))}
            </div>
...
// sub-subtasks (layer 2).
function SubtaskRow({
  subtask, onUpdate, onDelete, onAddSubSubtask, onUpdateSubSubtask, onDeleteSubSubtask,
  currentTaskId, allTasksFlat, allSubtasksFlat,
  onPromoteToTask, onMoveToTask, onMoveToSubtask,
  onPromoteSubSubtaskToSubtask, onMoveSubSubtaskUnderSubtask,
}) {
  const [expanded, setExpanded] = useState(false);
  const [subSubText, setSubSubText] = useState('');
  const [notes, setNotes] = useState(subtask.notes || '');
  const [driveLink, setDriveLink] = useState(subtask.driveLink || '');
  ...
  // Nesting one level deeper (becoming a sub-subtask) is only safe if this
  // subtask has no sub-subtasks of its own to lose.
  const canNestDeeper = subsubtasks.length === 0;
  const otherTasksForMove = (allTasksFlat || []).filter(t => t.id !== currentTaskId);
  const otherSubtasksForNest = (allSubtasksFlat || []).filter(s => s.id !== subtask.id);

  useEffect(() => { setNotes(subtask.notes || ''); }, [subtask.id]);
  useEffect(() => { setDriveLink(subtask.driveLink || ''); }, [subtask.id]);
  ...
  function handleMove(e) {
    ...
    e.target.value = '';
    if (!val) return;
    if (val === 'promote') { onPromoteToTask(); return; }
    const [kind, a, b] = val.split(':');
    if (kind === 'task') onMoveToTask(a);
    else if (kind === 'subtask') onMoveToSubtask(a, b);
  }
        ...
        <div className="px-2 pb-2 pt-1 border-t flex flex-col gap-1.5" style={{ borderColor: COLORS.border }}>
          <div className="flex gap-2 flex-wrap">
            <div className="flex-1 min-w-[140px]">
              <label className="wb-mono text-[10px] uppercase tracking-wide" style={{ color: COLORS.inkSoft }}>Drive link</label>
            ...
            <div className="flex-1 min-w-[140px]">
              <label className="wb-mono text-[10px] uppercase tracking-wide" style={{ color: COLORS.inkSoft }}>Move</label>
              <select
                value=""
                onChange={handleMove}
                title="Move this subtask elsewhere in the outline"
                className="w-full mt-1 px-2 py-1 rounded-md border text-[11px] outline-none"
                style={{ borderColor: COLORS.border, color: COLORS.ink }}
              >
                <option value="" disabled>Move…</option>
                <option value="promote">↑ Promote to task</option>
                {otherTasksForMove.length > 0 && (
                  <optgroup label="Move to task">
                    {otherTasksForMove.map(t => (
                      <option key={`task:${t.id}`} value={`task:${t.id}`}>↳ {t.text}</option>
                    ))}
                  </optgroup>
                )}
                {canNestDeeper && otherSubtasksForNest.length > 0 && (
                  <optgroup label="Nest under subtask">
                    {otherSubtasksForNest.map(s => (
                      <option key={`subtask:${s.taskId}:${s.id}`} value={`subtask:${s.taskId}:${s.id}`}>↳↳ {s.taskText} → {s.text}</option>
                    ))}
                  </optgroup>
                )}
              </select>
          ...
          {subsubtasks.map(ss => (
            <SubSubtaskRow
              ...
              onPromoteToSubtask={() => onPromoteSubSubtaskToSubtask(ss.id)}
              onMoveToSubtask={(targetTaskId, targetSubId) => onMoveSubSubtaskUnderSubtask(ss.id, targetTaskId, targetSubId)}
            />
          ))}
...
function SubSubtaskRow({ subsubtask, onUpdate, onDelete, allSubtasksFlat, currentSubtaskId, onPromoteToSubtask, onMoveToSubtask }) {
  const [expanded, setExpanded] = useState(false);
  const [notes, setNotes] = useState(subsubtask.notes || '');
  const [driveLink, setDriveLink] = useState(subsubtask.driveLink || '');
  const state = getItemState(subsubtask);
  const priority = getItemPriority(subsubtask);
  const due = formatDue(subsubtask.dueDate);
  const overdue = due && due.overdue && state !== 'Done';
  const otherSubtasksForMove = (allSubtasksFlat || []).filter(s => s.id !== currentSubtaskId);

  useEffect(() => { setNotes(subsubtask.notes || ''); }, [subsubtask.id]);
  useEffect(() => { setDriveLink(subsubtask.driveLink || ''); }, [subsubtask.id]);
  ...
  function handleMove(e) {
    ...
    e.target.value = '';
    if (!val) return;
    if (val === 'promote') { onPromoteToSubtask(); return; }
    const [, a, b] = val.split(':');
    onMoveToSubtask(a, b);
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-2 pl-4 flex-wrap">
        ...
        <PrioritySelect value={priority} onChange={p => onUpdate({ priority: p })} />
        <StateSelect value={state} onChange={s => onUpdate({ state: s })} />
        {subsubtask.driveLink && (
          <a
            href={subsubtask.driveLink}
            target="_blank"
            rel="noopener noreferrer"
            title="Open linked document"
            style={{ color: COLORS.sky }}
      ...
      {expanded && (
        <div className="pl-9 pr-1 pt-1 pb-1 flex flex-col gap-1.5">
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            onBlur={() => onUpdate({ notes })}
          ...
          <div className="flex gap-1.5 flex-wrap">
            ...
            <select
              value=""
              onChange={handleMove}
              title="Move this sub-subtask elsewhere in the outline"
              className="flex-1 min-w-[130px] px-2 py-1 rounded-md border text-[11px] outline-none"
              style={{ borderColor: COLORS.border, color: COLORS.ink }}
            >
              <option value="" disabled>Move…</option>
              <option value="promote">↑ Promote to subtask</option>
              {otherSubtasksForMove.length > 0 && (
                <optgroup label="Move under subtask">
                  {otherSubtasksForMove.map(s => (
                    <option key={`subtask:${s.taskId}:${s.id}`} value={`subtask:${s.taskId}:${s.id}`}>↳ {s.taskText} → {s.text}</option>
                  ))}
                </optgroup>
              )}
