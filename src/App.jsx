  ...
  function addSubtask(projectId, taskId, text) {
    ...
    persistProjects(projects.map(p => {
      ...
      return {
        ...
        tasks: p.tasks.map(t => (
          t.id === taskId
            ? { ...t, subtasks: [...(t.subtasks || []), { id: uid(), text: text.trim(), state: 'Not Scheduled', priority: 'No Priority', dueDate: '', subtasks: [] }] }
            : t
        )),
      };
    }));
        ...
        tasks: p.tasks.map(t => {
          ...
          return {
            ...
            subtasks: (t.subtasks || []).map(s => (
              s.id === subtaskId
                ? { ...s, subtasks: [...(s.subtasks || []), { id: uid(), text: text.trim(), state: 'Not Scheduled', priority: 'No Priority', dueDate: '' }] }
                : s
            )),
          };
        }),
...
function TreeRow({ item, depth, onToggle, onEditText, onEditState, onEditPriority }) {
      ...
      {expanded && (
        <div className="px-2 pb-2 pt-1 border-t flex flex-col gap-1.5" style={{ borderColor: COLORS.border }}>
          {subsubtasks.map(ss => {
            ...
            const ssPriority = getItemPriority(ss);
            const ssDue = formatDue(ss.dueDate);
            const ssOverdue = ssDue && ssDue.overdue && ssState !== 'Done';
            return (
              <div key={ss.id} className="flex items-center gap-2 pl-4 flex-wrap">
                ...
                <EditableText
                  ...
                  style={{ color: ssState === 'Done' ? COLORS.inkSoft : COLORS.ink, textDecoration: ssState === 'Done' ? 'line-through' : 'none' }}
                />
                <input
                  type="date"
                  value={ss.dueDate || ''}
                ...
                />
                <PrioritySelect value={ssPriority} onChange={p => onUpdateSubSubtask(ss.id, { priority: p })} />
