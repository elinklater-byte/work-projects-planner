...
export default function WorkBoard() {
  ...
  function addTask(projectId, text) {
    ...
    persistProjects(projects.map(p => (
      p.id === projectId
        ? { ...p, tasks: [...p.tasks, { id: uid(), text: text.trim(), done: false, dueDate: '', notes: '', subtasks: [] }] }
        : p
    )));
  }
    ...
    <div className="min-h-screen" style={{ backgroundColor: COLORS.paper, color: COLORS.ink }}>
      ...
      {activeProject && (
        <ProjectDetail
          ...
          onToggleSubtask={(taskId, subId) => toggleSubtask(activeProject.id, taskId, subId)}
      ...
      <div className="px-3 pt-3 pb-2" onClick={onOpen}>
        ...
        {totalCount > 0 && (
          <div className="mb-2">
            ...
            <div className="wb-mono text-xs mt-1" style={{ color: COLORS.inkSoft }}>{doneCount}/{totalCount} tasks</div>
          </div>
        )}
        {due && (
...
function ProjectDetail({ project, onClose, onUpdate, onDelete, onAddTask, onToggleTask, onDeleteTask, onUpdateTask, onAddSubtask, onToggleSubtask, onDeleteSubtask }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(35,36,32,0.5)' }} onClick={handleClose}>
      ...
      >
        ...
        {due && (
          <div className="wb-mono text-xs mb-4" style={{ color: overdue ? COLORS.rose : COLORS.inkSoft }}>
            {overdue
...
function TaskItem({ task, onToggle, onDelete, onUpdate, onAddSubtask, onToggleSubtask, onDeleteSubtask }) {
  ...
  const overdue = due && due.overdue && !task.done;
  ...
  return (
    <div className="rounded-lg border" style={{ borderColor: COLORS.border }}>
      <div className="flex items-center gap-2 px-2 py-2 group">
        ...
        >{task.text}</span>
        {subtasks.length > 0 && (
          <span className="wb-mono text-xs" style={{ color: COLORS.inkSoft }}>{subDone}/{subtasks.length}</span>
        )}
        {due && (
              ...
              {subtasks.map(s => (
                <div key={s.id} className="flex items-center gap-2 pl-1">
                  ...
                  <span
                    ...
                    style={{ color: s.done ? COLORS.inkSoft : COLORS.ink, textDecoration: s.done ? 'line-through' : 'none' }}
                  >{s.text}</span>
                  <button onClick={() => onDeleteSubtask(s.id)}>
